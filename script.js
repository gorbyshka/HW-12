...
for (let i = 0; i < arr.length; i++) {
    if (arr[i] % 2 !== 0) {
        positive.push(arr[i]);
        result = positive.filter(item => item > 0);
    }
}
...
for (let i = 0; i < arr.length; i++) {
    if (arr[i] % 2 == 0) {
        posPair.push(arr[i]);
        result = posPair.filter(item => item > 0);
    }
}

console.log('Количество парных положительных элементов' + ': ' + result.length);
console.log('Сумма парных положительных элементов' + ': ' + result.map(i => x += i, x = 0).reverse()[0]);
